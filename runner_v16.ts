/**
 * Bond Vigilante — Backtest v16
 * ATR Compression Breakout ("Squeeze")
 * - When 3-bar ATR compresses to < 70% of 20-bar ATR → market building energy
 * - Breakout of compression range with volume → trend continuation
 * - Clean 1:2 R:R → PF = (0.60×2)/(0.40×1) = 3.0 theoretical
 * - Plus original NearIB (best signal) + M1 20-bar break (frequency)
 * - Target: 50-70 trades per pair, WR 60%+, PF 2.5+, Sortino 0.4+
 */
...
function getSignal(bars: any[], i: number,
  ...
  // ── SQUEEZE BREAKOUT ─────────────────────────────────
  // ATR compression: 3-bar mean range < 70% of 20-bar ATR
  {
    const atr3=bars.slice(i-3,i).reduce((s:number,b:any)=>s+(b.h-b.l),0)/3;
    const squeezed = atr3 < atrArr[i]*0.72;
    if(squeezed) {
      // High of consolidation range (last 4 bars excluding current)
  ...
  // ── 20-BAR BREAKOUT (moderate conditions) ───────────
  {
...
async function main() {
  console.log("\n📊 Bond Vigilante v16 — Squeeze + NearIB + 20Bar (Clean 1:2 R:R)\n");
