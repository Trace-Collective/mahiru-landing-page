/**
 * Bond Vigilante — Backtest v24
 * MAXIMUM LEVERAGE: BTC/ETH 20×, SOL 15×
 * Hyperliquid supports up to 50× — finding the practical ceiling
 *
 * At 20× BTC: SL = 1×ATR ≈ 1.6% → loss = 32% per trade
 * Liquidation at 5% adverse move (1/20) — ATR ≈ 1.6%, so liquidation = 3.1×ATR
 * Still fires before liquidation in simulation (SL at 1×ATR = 1/3 of liquidation distance)
 *
 * Risk stops: proportionally scaled
 *   daily: -18% per pair  |  weekly: -30%
 */
...
function getSignal(bars: any[], i: number,
  ...
  const sFund=f<-0.000010,lFund=f>0.000008;
  {
    ...
    if(atr3 < atrArr[i]*0.72) {
      const sqHigh=Math.max(...bars.slice(i-4,i).map((b:any)=>b.h));
      const sqLow =Math.min(...bars.slice(i-4,i).map((b:any)=>b.l));
      if(reg==="bull"&&c.c>sqHigh&&c.c>c.o&&vr>1.8&&r>52&&r<78&&!lFund) return {side:"long",setup:"Q-Squeeze"};
      if(reg==="bear"&&c.c<sqLow&&c.c<c.o&&vr>1.8&&r>22&&r<48&&!sFund) return {side:"short",setup:"Q-Squeeze"};
    }
  }
  {
    const p2r=p2.h-p2.l,pr=p.h-p.l;
    if(pr<p2r*0.68&&p.h<=p2.h*1.001&&p.l>=p2.l*0.999) {
      if(reg==="bull"&&c.c>p2.h*0.999&&c.c>c.o&&vr>1.7&&r>48&&r<75&&!lFund) return {side:"long",setup:"N-NearIB"};
      if(reg==="bear"&&c.c<p2.l*1.001&&c.c<c.o&&vr>1.7&&r>25&&r<52&&!sFund) return {side:"short",setup:"N-NearIB"};
    }
  }
  {
    const h20=Math.max(...bars.slice(i-20,i-2).map((b:any)=>b.h));
    const l20=Math.min(...bars.slice(i-20,i-2).map((b:any)=>b.l));
    if(reg==="bull"&&c.c>h20&&c.c>c.o&&r>58&&r<78&&vr>1.9&&!lFund) return {side:"long",setup:"B-20Bar"};
    if(reg==="bear"&&c.c<l20&&c.c<c.o&&r>22&&r<42&&vr>1.9&&!sFund) return {side:"short",setup:"B-20Bar"};
  }
  return null;
}
...
async function runAll() {
  ...
  const lev:Record<string,number>={BTC:20,ETH:20,SOL:15};
  ...
  for(let i=80;i<minLen;i++){
    ...
    if(wPnl<-30.0){pauseUntil=i+7;continue;}
    if(gCons>=3){pauseUntil=i+6;gCons=0;continue;}

    for(const pair of PAIRS){
      if(pos.has(pair)||dPnl[pair]<-18||dCons[pair]>=2||pos.size>=2)continue;
      const d=D[pair];
...
async function main() {
  console.log("\n📊 Bond Vigilante v24 — 20×/15× — MAXIMUM LEADERBOARD\n");
  ...
  console.log(`  v16→80.9  v21→178.7  v22→215.5  v23→323.0  v24→${m.lb}\n`);
}
