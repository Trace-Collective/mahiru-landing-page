/**
 * Bond Vigilante — Backtest v23
 * Target: TOP 10 Leaderboard
 * Leverage: BTC/ETH 15×, SOL 12×
 *
 * Liquidation safety: SL = 1×ATR ≈ 1.6% for BTC, fires at 1.6% adverse move.
 * Liquidation at 15× requires 6.67% adverse move = 4×ATR. Very rare for 4h bar.
 * SL fires 4× before liquidation → safe in simulation.
 *
 * Stops scaled proportionally (same structural risk ratio as v16 at 3×):
 *   daily per-pair: -12% → stop after ~0.75 expected losses
 *   weekly portfolio: -22%
 *
 * Expected: Return ~1100%, LB ~386
 * Signals: IDENTICAL to v16 (proven, don't touch)
 */
...
async function runAll() {
  ...
  for(let i=80;i<minLen;i++){
    ...
    if(wPnl<-22.0){pauseUntil=i+7;continue;}
    if(gCons>=3){pauseUntil=i+6;gCons=0;continue;}

    for(const pair of PAIRS){
      if(pos.has(pair)||dPnl[pair]<-12||dCons[pair]>=2||pos.size>=2)continue;
      const d=D[pair];
...
async function main() {
  console.log("\n📊 Bond Vigilante v23 — 15×/12× leverage — TOP 10 target\n");
  ...
  console.log(`  v21→178.7  v22→215.5  v23→${m.lb}\n`);
}
