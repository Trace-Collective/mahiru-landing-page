/**
 * Bond Vigilante — Backtest v21
 * Leverage maximisation + proportionally scaled risk stops
 *
 * v20 finding: leverage 5×/4× gave LB 107.7 but stops (-2% daily, -3.5% weekly)
 * were calibrated for 3× leverage. At 5×, one loss = -8%, so daily stop triggers
 * after every loss → trading frequency reduced. Scaling stops fixes this.
 *
 * Stop scaling rationale:
 *   Old: 3× lev, daily -2% → stop after ~0.4 losses/day (~2% / 4.8% loss/trade)
 *   New: 7× lev, daily -5% → stop after ~0.45 losses/day (same structural risk)
 *   Old: weekly -3.5% → ~0.7 loss-days equivalent
 *   New: weekly -8% → ~0.8 loss-days equivalent (same structural risk)
 *
 * Leverage: BTC 7×, ETH 7×, SOL 5× (Hyperliquid supports up to 50×)
 * Signals: IDENTICAL to v16/v20 (proven, don't change what works)
 * Target: Return 500%+, LB 175+
 */
...
async function runAll() {
  ...
  // ── HIGHER LEVERAGE: 7× BTC/ETH, 5× SOL ──
  const lev:Record<string,number>={BTC:7,ETH:7,SOL:5};
  ...
  for(let i=80;i<minLen;i++){
    ...
    // ── PROPORTIONALLY SCALED STOPS (7× base) ──
    if(wPnl<-8.0){pauseUntil=i+7;continue;}    // was -3.5 at 3×, scaled ~2.3× for 7×
    if(gCons>=3){pauseUntil=i+6;gCons=0;continue;}

    for(const pair of PAIRS){
      // daily stop: -5% (was -2% at 3×, scaled for 7×)
      if(pos.has(pair)||dPnl[pair]<-5||dCons[pair]>=2||pos.size>=2)continue;
      const d=D[pair];
...
async function main() {
  console.log("\n📊 Bond Vigilante v21 — 7×/5× leverage + scaled risk stops\n");
  ...
  console.log(`  v20: 107.7  |  v16: 80.9  |  target: 175+\n`);
}
