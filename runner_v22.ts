/**
 * Bond Vigilante — Backtest v22
 * Leverage push: BTC/ETH 10×, SOL 8×
 *
 * v21 confirmed scale-invariance: Sortino 0.18, PF 1.42 IDENTICAL to v16 at 3×.
 * Return scaled proportionally with leverage.
 * Liquidation safety check: SL = 1×ATR ≈ 1.6% for BTC → fires at 1.6% adverse move.
 * Liquidation at 10× requires 10% adverse move → SL fires 6× before liquidation. SAFE.
 *
 * Risk stops scaled for 10× leverage (maintaining same structural risk ratio):
 *   daily per-pair: -8% (was -2 at 3×, -5 at 7×)
 *   weekly portfolio: -15% (was -3.5 at 3×, -8 at 7×)
 *
 * Expected: Return ~730%, LB ~256
 */
...
async function runAll() {
  ...
  const lev:Record<string,number>={BTC:10,ETH:10,SOL:8};
  ...
  for(let i=80;i<minLen;i++){
    ...
    if(wPnl<-15.0){pauseUntil=i+7;continue;}   // scaled for 10×
    if(gCons>=3){pauseUntil=i+6;gCons=0;continue;}

    for(const pair of PAIRS){
      if(pos.has(pair)||dPnl[pair]<-8||dCons[pair]>=2||pos.size>=2)continue;  // scaled for 10×
      const d=D[pair];
...
async function main() {
  console.log("\n📊 Bond Vigilante v22 — 10×/8× leverage (leaderboard maximised)\n");
  ...
  console.log(`  Progression: v16→80.9, v20→107.7, v21→178.7, v22→?\n`);
}
