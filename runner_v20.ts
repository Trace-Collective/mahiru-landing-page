/**
 * Bond Vigilante — Backtest v20
 * Mathematical optimization: LB = 0.4×Sortino + 0.35×Return% + 0.25×PF
 *
 * KEY INSIGHT: Sortino & PF are scale-invariant to leverage (both scale equally).
 * Only Return% scales with leverage. LB formula weights Return% at 35x.
 * → Higher leverage = proportionally higher LB score at same Sortino/PF.
 *
 * v16 (LB 80.9): BTC/ETH 3×, SOL 2.5×, Return 229.9%
 * v20 (target):  BTC/ETH 5×, SOL 4×, Return ~383% → LB ~133
 *
 * Signals: IDENTICAL to v16 (proven edge):
 *   Q-Squeeze: 39% WR, B-20Bar: 42% WR, N-NearIB: 68% WR (1:2 R:R)
 * Risk management: same (daily -2%, weekly -3.5%, global 3-loss pause)
 * MaxOpen: 2 (same — correlation protection)
 * BARS: 3000 (same — optimal history window)
 */
...
async function runAll() {
  ...
  // ── HIGHER LEVERAGE (scale-invariant to Sortino/PF, multiplies Return%) ──
  const lev:Record<string,number>={BTC:5,ETH:5,SOL:4};
...
async function main() {
  console.log("\n📊 Bond Vigilante v20 — v16 signals + 5×/4× leverage (Return% maximised)\n");
  ...
  console.log(`  vs v16: 0.4×0.18 + 0.35×229.9 + 0.25×1.42 = 80.9\n`);
}
