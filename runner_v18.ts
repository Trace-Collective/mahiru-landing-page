/**
 * Bond Vigilante — Backtest v18
 * 4-Pair Expansion: BTC + ETH + SOL + AVAX
 * Key insight: LB = 0.4×Sortino + 0.35×Return% + 0.25×PF
 * Return% dominates (35× multiplier). Same WR/PF but 33% more trades → ~33% more Return%
 * v16 LB: 80.9 → target v18: 105+
 *
 * Changes from v16 (LB 80.9):
 * - ADD AVAX as 4th pair (high vol L1, similar momentum characteristics)
 * - MaxOpen = 3 (up from 2, to allow 3 concurrent of 4 pairs)
 * - Leverage: AVAX 2.5× (same as SOL)
 * - Signals: IDENTICAL to v16 (N-NearIB + Q-Squeeze + B-20Bar proven edge)
 * - Weekly stop: -4.5% (slightly looser — more capacity to let winners run)
 */
...
async function runAll() {
  ...
  const dPnl:Record<string,number>={BTC:0,ETH:0,SOL:0,AVAX:0};
  const dCons:Record<string,number>={BTC:0,ETH:0,SOL:0,AVAX:0};
  const lev:Record<string,number>={BTC:3,ETH:3,SOL:2.5,AVAX:2.5};
  ...
  for(let i=80;i<minLen;i++){
    ...
    if(wPnl<-4.5){pauseUntil=i+7;continue;}
    if(gCons>=3){pauseUntil=i+6;gCons=0;continue;}

    for(const pair of PAIRS){
      if(pos.has(pair)||dPnl[pair]<-2||dCons[pair]>=2||pos.size>=3)continue;
      const d=D[pair];
...
async function main() {
  console.log("\n📊 Bond Vigilante v18 — 4 Pairs: BTC+ETH+SOL+AVAX (same signals as v16)\n");
  const allTrades=await runAll();
  const pp:Record<string,Trade[]>={BTC:[],ETH:[],SOL:[],AVAX:[]};
